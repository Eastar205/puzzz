...
2. **캐릭터 프리팹/오브젝트 4종** (전사/마법사/궁수/힐러)
   ...
   - 상단 화면에 배치 후 `CharacterRoster`의 4개 슬롯에 연결
3. **몬스터 프리팹**
...
4. **메인 게임 씬** (`Assets/Scenes/Main.unity`)
   ...
   - 상단 전투 영역: `CharacterRoster`(4명 연결), `WaveManager`(몬스터 프리팹/스폰 위치/기지 위치 연결), `BattleManager` 배치
...
- 몬스터는 웨이브 단위로 스폰되어 기지로 이동, 캐릭터 자동 공격/스킬로 저지 (`WaveManager`, `BattleManager`)
- 기지 체력 0 → 실패, 웨이브 전멸 → 클리어
